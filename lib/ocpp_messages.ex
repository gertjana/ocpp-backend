defmodule OcppMessages do
  ...
  def handle([2, id, "BootNotification", _], req, state) do
    {:ok, reply} = JSEX.encode([3,id, [status: "Accepted", currentTime: Utils.time_as_string, heartbeatInterval: 300]])
    {:reply, {:text, reply}, req, state}
  end

  def handle([2, id, "Heartbeat"], req, state) do
    {:ok, reply} = JSEX.encode([3, id, [currentTime: Utils.time_as_string]])
    {:reply, {:text, reply}, req, state}
  end
